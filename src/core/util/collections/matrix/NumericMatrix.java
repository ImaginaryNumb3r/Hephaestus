...
public class NumericMatrix<T extends Number> extends MutatingMatrix<T> {
    ...
    public NumericMatrix(@NotNull T[][] matrix, @NotNull BiSupplier<T, Number> numberSupplier) {
        super(matrix);
        _numberSupplier = numberSupplier;
    }

    public NumericMatrix(@NotNull Tuple<Integer, Integer> bounds, @NotNull BiSupplier<T, Number> numberSupplier) {
        super(bounds);
        _numberSupplier = numberSupplier;
    }

    public NumericMatrix(int width, int height, @NotNull BiSupplier<T, Number> numberSupplier) {
        super(width, height);
        _numberSupplier = numberSupplier;
    }
    ...
    protected void computeMultiplication(Matrix<T> other, T[][] matrix){
        ...
        BiFunction<T, T, T> multiplication = (val1, val2) -> {
            ...
            return _numberSupplier.make(double1 + double2);
        };
